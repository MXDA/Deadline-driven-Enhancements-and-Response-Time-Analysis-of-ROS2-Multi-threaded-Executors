...
int main(int argc, char **argv) {
	...
	auto parameters_client = std::make_shared<rclcpp::SyncParametersClient>(node);
  	...
  	int COUNT_MAX = parameters_client->get_parameter("count_max", 5);
    ...
    /*
    std::vector<uint64_t> chain_lengths = {2};
	std::vector<std::vector<uint64_t>> chain_member_ids = {{1, 2}};

	std::vector<double_t> node_runtimes = {50, 60};
	std::vector<uint64_t> chain_periods = {100};
	std::vector<uint64_t> chain_deadlines = {250};
    */
    std::vector<uint64_t> chain_lengths = {2, 4};
	std::vector<std::vector<uint64_t>> chain_member_ids = {{1, 2}, {3, 4, 5, 6}};

	std::vector<double_t> node_runtimes = {50, 60, 15, 25, 35, 45};
	...
	for(uint i = 0; i < chain_lengths.size(); ++i) {
		...
		chain_runtimes.push_back(sum - node_runtimes[index - 1]);
	}
